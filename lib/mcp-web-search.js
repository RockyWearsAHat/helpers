  ...
  async function searchWeb(args) {
    ...
    for (let attempt = 0; attempt < GOOGLE_EMPTY_RETRY_MAX; attempt++) {
      ...
      if (firstOutcome.challenge) {
        ...
        // 1) Reuse an already human-verified Chrome session if one exists, so a
        //    single "I am not a robot" solve covers every later search/page —
        //    no re-prompt. Returns null when no verified session is available.
        let solved = await searchViaVerifiedChrome(searchUrls[0]);

        // 2) No verified session yet (or it just expired) — prompt the user to
        //    solve the CAPTCHA once in the visible browser. That browser is then
        //    kept open and reused for all subsequent searches this session.
        if (!solved || solved.challenge) {
          ...
          const challengeOutcome =
            await resolveGoogleChallengeViaLiveChrome(searchUrls[0]);
          if (challengeOutcome.challenge) {
            throw new Error(
              "Google presented a CAPTCHA in the interactive browser. Solve it, then retry the search.",
            );
          }
          // A concurrent query may have shared this solve; if it resolved a
          // different URL, re-run our own query through the now-verified browser.
          if (
            challengeOutcome.resolvedUrl &&
            challengeOutcome.resolvedUrl !== searchUrls[0]
          ) {
            solved =
              (await searchViaVerifiedChrome(searchUrls[0])) || challengeOutcome;
          } else {
            solved = challengeOutcome;
          }
        }

        // The verified browser navigated to our own query, so its results are
        // the answer. Use them directly instead of re-fetching via headless
        // (which would just hit the CAPTCHA again). If it cleared the challenge
        // but parsed no results, retry headless — the cookies are now valid.
        if (solved && Array.isArray(solved.results) && solved.results.length > 0) {
          results = mergeGoogleResults(results, solved.results, maxResults);
        } else {
          continue;
        }
      }
