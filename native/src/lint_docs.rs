...
/// The classifier an ungroundable language reads through: the shared store when this machine has
/// grounded anything, else the committed bootstrap (`lint-index/polarity-bootstrap.json` — itself
/// machine-generated by grounding, shipped like every other learned artifact).
fn transferred_polarity(data_root: &Path) -> Option<crate::lint_read::Polarity> {
    let stored = std::fs::read_to_string(global_polarity_path())
        .ok()
...
mod transfer_probe {
    ...
    /// already read (the cached python memory), re-grounded against the local toolchain, plus the
    /// shipped corpus prose. Machine-generated end to end; run after learning-path changes:
    /// `cargo test --release generate_polarity_bootstrap -- --ignored`
    #[test]
    ...
    fn generate_polarity_bootstrap() {
        ...
        let data_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap().to_path_buf();
        let cached = crate::lint_train::model_dir_pub().join("python.learned.json");
        let json: serde_json::Value =
            serde_json::from_str(&std::fs::read_to_string(&cached).expect("cached python memory (run a lint first)"))
                .expect("catalog parses");
        let bindings = json["memory"]["bindings"].as_array().expect("memory bindings");

        let mut reader = Reader::new();
        for b in bindings {
            reader.learn_span(b["prose"].as_str().unwrap_or(""));
        }
        let mut builder = PolarityBuilder::new(reader);
        for prose in crate::lint_train::corpus_prohibition_prose(&data_root) {
            builder.accumulate(&prose, true);
        ...
        for b in bindings {
            let (prose, code) = (b["prose"].as_str().unwrap_or(""), b["code"].as_str().unwrap_or(""));
            if prose.split_whitespace().count() < 3 || code.len() < 3 {
                continue;
            }
            match crate::lint_toolchain::check("python", code, &data_root) {
                crate::lint_toolchain::Verdict::Flagged => builder.accumulate(prose, true),
                crate::lint_toolchain::Verdict::Clean => builder.accumulate(prose, false),
